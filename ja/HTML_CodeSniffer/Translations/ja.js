_global.translation['ja'] = {
    ...
    , "1_3_1_H63.2": '他の要素の見出しとして機能する td 要素の scope 属性は、HTML5では廃止されました。代わりに th 要素を使用してください。'
    , "1_3_1_H43.ScopeAmbiguous": '複数のレベルの見出しを持つテーブルでは、 th 要素の scope 属性はあいまいです。代わりに td 要素の headers 属性を使用してください。'
    , "1_3_1_H43.IncorrectAttr": 'この td 要素の headers 属性が正しくありません。 "{{expected}}" を予期していましたが、 "{{actual}}" を検出しました'
    , "1_3_1_H43.IncorrectAttrNotice": 'td 要素の headers 属性が正しいか確認してください。'
    ...
    //1_3_4.js
    , "1_3_4.RestrictView": '特定の表示方向が不可欠な場合を除き、コンテンツの表示や操作が縦長や横長などの単一の表示方向に制限されていないことを確認してください。'


    //1_3_5.js
    , "1_3_5_H98.FaultyValue": 'この要素のオートコンプリート属性には、不正な値が含まれている可能性があります。: {{valuesStr}}.'
    , "1_3_5_H98.InvalidAutocomplete_Text": '不正なオートコンプリート属性の値: {{x}}. 要素はテキストコントロールグループに属していません。'
    , "1_3_5_H98.InvalidAutocomplete_Multiline": '不正なオートコンプリート属性の値: {{x}}. 要素は複数行のコントロールグループに属していません。'
    , "1_3_5_H98.InvalidAutocomplete_Password": '不正なオートコンプリート属性の値: {{x}}. 要素はパスワードコントロールグループに属していません。'
    , "1_3_5_H98.InvalidAutocomplete_Url": '不正なオートコンプリート属性の値: {{x}}. 要素は URL コントロールグループに属していません。'
    , "1_3_5_H98.InvalidAutocomplete_Telephone": '不正なオートコンプリート属性の値: {{x}}. 要素は電話番号コントロールグループに属していません。'
    , "1_3_5_H98.InvalidAutocomplete_Numeric": '不正なオートコンプリート属性の値: {{x}}. 要素は数値コントロールグループに属していません。'
    , "1_3_5_H98.InvalidAutocomplete_Month": '不正なオートコンプリート属性の値: {{x}}. 要素は年月コントロールグループに属していません。'
    , "1_3_5_H98.InvalidAutocomplete_Date": '不正なオートコンプリート属性の値: {{x}}. 要素は日付コントロールグループに属していません。'
    , "1_3_5_H98.Purpose": '入力フィールドが「ユーザーインターフェースコンポーネントの入力目的」で特定された目的を果たすものであるかどうか、また、フォームの入力データに対して想定される意味の特定をサポートする技術を用いてコンテンツが実装されているかどうかを確認してください。'
    , "1_3_5_H98.MissingAutocomplete": 'この要素にはオートコンプリート属性がありません。このフィールドがユーザーに関する情報を収集する場合は、この達成基準を満たすために属性の追加を検討してください。'


    //1_3_6.js
    , "1_3_6_ARIA11.Check": 'ユーザーインターフェースコンポーネント、アイコン、領域の目的がプログラムで決定できることを確認してください。'
    ...
    //1_4_3.js
    , "1_4_3_G18_or_G145.Abs": 'この要素は絶対位置に配置されているため、背景色を決定できません。テキストと背景の覆われた部分すべてのコントラスト比が少なくとも {{required}}:1 であることを確認してください。'
    , "1_4_3_G18_or_G145.BgImage": 'この要素のテキストは背景画像に配置されます。テキストと画像の覆われている部分すべてのコントラスト比が少なくとも {{required}}:1 であることを確認してください。'
    , "1_4_3_G18_or_G145.Alpha": 'この要素のテキストまたは背景は透明部分を含みます。テキストと背景のコントラスト比が少なくとも {{required}}:1 であることを確認してください。'
    , "1_4_3_G18_or_G145.Fail": 'この要素は、この適合レベルではコントラストが不十分です。少なくとも {{required}}:1 のコントラスト比が必要ですが、この要素のテキストのコントラスト比は {{value}}:1 です。'
    ...
    //1_4_6.js
    , "1_4_6_G18_or_G17.Abs": 'この要素は絶対位置に配置されているため、背景色を決定できません。テキストと背景の覆われた部分すべてのコントラスト比が少なくとも {{required}}:1 であることを確認してください。'
    , "1_4_6_G18_or_G17.BgImage": 'この要素のテキストは背景画像に配置されます。テキストと画像の覆われている部分すべてのコントラスト比が少なくとも {{required}}:1 であることを確認してください。'
    , "1_4_6_G18_or_G17.Fail": 'この要素は、この適合レベルではコントラストが不十分です。少なくとも {{required}}:1 のコントラスト比が必要ですが、この要素のテキストのコントラスト比は {{value}}:1 です。'
    ...
    //1_4_10.js
    , "1_4_10_C32,C31,C33,C38,SCR34,G206.Check": 'コンテンツは情報や機能を損なうことなく、以下において二次元でのスクロールを必要とせずに表示できることを確認してください。\
        幅が 320 CSS ピクセルに相当する縦スクロールのコンテンツ。\
        高さが 256 CSS ピクセルに相当する横スクロールのコンテンツ\
        ただし、利用や意味を理解するために二次元のレイアウトが必須である一部のコンテンツを除きます。'
    , "1_4_10_C32,C31,C33,C38,SCR34,G206.Fixed": 'この要素は "position: fixed" が指定されているため、二次元にスクロールする必要があるかもしれませんが、これはこの達成基準においてはエラーとみなされます。'
    , "1_4_10_C32,C31,C33,C38,SCR34,G206.Scrolling": 'プリフォーマットされたテキストは、二次元にスクロールする必要がある場合がありますが、これはこの達成基準においてはエラーとみなされます。'
    , "1_4_10_C32,C31,C33,C38,SCR34,G206.Zoom": 'ユーザーエージェントのズーム機能を妨害することは、この達成基準においてエラーとなる可能性があります。'


    //1_4_11.js
    , "1_4_11_G195,G207,G18,G145,G174,F78.Check": '隣接する色とのコントラスト比が 3:1 以上であることを確認してください。ただし下記の場合は除きます。\
    ユーザーインターフェースコンポーネント: ユーザインタフェース コンポーネントおよび状態（state）を特定するのに必要な視覚的な情報。ただし、アクティブではないユーザインタフェース コンポーネントや、そのコンポーネントの見た目がユーザエージェントによって提示されていてコンテンツ制作者が変更していない場合。\
    グラフィカルオブジェクト: コンテンツを理解するために必要なグラフィックス部分。ただし、グラフィックスの特定の表示が伝達される情報にとって不可欠な場合。'


    //1_4_12.js
    , "1_4_12_C36,C35.Check": '以下のすべてを設定し、他のスタイルプロパティを変更しないことで、コンテンツや機能が損なわれないことを確認します。\
    ...
    //1_4_13.js
    , "1_4_13_F95.Check": 'ポインタのホバーやキーボードのフォーカスを受け取ってから外すことで、追加コンテンツを表示させてから非表示にする場合、以下のようになっていることを確認してください。\
    非表示にすることができる: ポインタのホバーやキーボードフォーカスを動かさずに追加コンテンツを非表示にするメカニズムが存在する。ただし、追加コンテンツが入力エラーを伝える場合や、他のコンテンツを不明瞭にしたり置き換えたりしない場合は除く。 \
    ホバーすることができる: ポインタのホバーによって追加コンテンツを表示させることができる場合、その追加コンテンツを消すことなく、ポインタを追加コンテンツ上で動かすことができる。 \
    表示が継続される: ホバーやフォーカスが解除される、利用者が非表示にする、またはその情報が有効でなくなるまでは、追加コンテンツが表示され続ける。'
    ...
    //2_1_4.js
    , "2_1_4.Check": '文字（大文字を小文字を含む）、句読点、数字、記号のみを使用したコンテンツにキーボードショートカットを実装する場合、以下のうち少なくとも1つが満たされていることを確認してください。 \
    解除: ショートカットを解除する仕組みがある。 \
    再割り当て: ショートカットを再割り当てして、1つまたは複数の非印刷文字（Ctrl、Alt など）を使用する仕組みがある。 \
    フォーカス中にのみ有効化: ユーザーインターフェースコンポーネントのキーボードショートカットは、そのコンポーネントにフォーカスがある場合にのみ有効になる。'


    //2_2_1.js
    , "2_2_1_F40.2": '別のページにリダイレクトするために使用される meta リフレッシュタグの制限時間が 0 ではありません。ユーザーはこの制限時間を制御できません。'
    ...
    //2_2_6.js
    , "2_2_6.Check": 'ユーザーが何も操作しない状態で 20 時間以上データが保存されている場合を除き、データ損失の原因となるユーザーの無操作の残り時間について、ユーザーに警告されることを確認してください。'
    ...
    //2_3_3.js
    , "2_3_3.Check": 'アニメーションが機能や情報伝達に不可欠なものでない限り、インタラクションによって引き起こされるモーションアニメーションを無効にできることを確認してください。'
    ...
    //2_5_1.js
    , "2_5_1.Check": 'マルチポイントまたは軌跡ベースのジェスチャを使って操作する機能は、マルチポイントまたは軌跡ベースのジェスチャが不可欠な場合を除き、軌跡ベースのジェスチャを使わずにシングルポインタで操作できることを確認してください。'


    //2_5_2.js
    , "2_5_2.SinglePointer_Check": 'シングルポインタで操作できる機能について、以下のうち少なくとも1つが満たされていることを確認してください。 \
        ダウンイベントがない: ポインタのダウンイベントが機能のどの部分の実行にも使われていない。 \
        中止または元に戻すことができる: 関数の完了はアップイベントであり、完了前に関数を中止したり、完了後に関数を元に戻したりする仕組みがある。 \
        アップイベントで反転: アップイベントは、先行するダウンイベントの結果を反転させるものである。 \
        必要不可欠: ダウンイベントで機能を完了させることが必須であること。'
    , "2_5_2.Mousedown_Check": 'この要素には mousedown イベントリスナーがあります。シングルポインタで操作できる機能については、以下のいずれかの条件が満たされていることを確認してください。 \
        ダウンイベントがない: ポインタのダウンイベントが機能のどの部分の実行にも使われていない。 \
        中止または元に戻すことができる: 関数の完了はアップイベントであり、完了前に関数を中止したり、完了後に関数を元に戻したりする仕組みがある。 \
        アップイベントで反転: アップイベントは、先行するダウンイベントの結果を反転させるものである。 \
        必要不可欠: ダウンイベントで機能を完了させることが必須であること。'
    , "2_5_2.Touchstart_Check": 'この要素には touchstart イベントリスナーがあります。シングルポインタで操作可能な機能については、以下のうち少なくとも1つが満たされていることを確認してください。 \
        ダウンイベントがない: ポインタのダウンイベントが機能のどの部分の実行にも使われていない。 \
        中止または元に戻すことができる: 関数の完了はアップイベントであり、完了前に関数を中止したり、完了後に関数を元に戻したりする仕組みがある。 \
        アップイベントで反転: アップイベントは、先行するダウンイベントの結果を反転させるものである。 \
        必要不可欠: ダウンイベントで機能を完了させることが必須であること。'
    ...
    //2_5_3.js
    , "2_5_3_F96.Check": 'テキストまたは文字画像を含むラベルを持つユーザーインターフェースコンポーネントでは、視覚的に表示されるテキストが名前（name）に含まれていることを確認してください。'
    , "2_5_3_F96.AccessibleName": 'この要素のアクセス可能な名前（name）には、視覚的に表示されるテキストが含まれていません。 テキストまたは文字画像を含むラベルを持つユーザーインターフェースコンポーネントでは、視覚的に表示されるテキストが名前（name）に含まれていることを確認してください。'


    //2_5_4.js
    , "2_5_4.Check": 'デバイスの動きやユーザーの動きで操作できる機能は、ユーザーインターフェースコンポーネントでも操作可能であり、誤って操作してしまうことを防ぐために、その動きへの反応を無効にできることを確認してください。ただし、以下の場合は例外とします。 \
        サポートされたインタフェース: アクセシビリティ サポーテッドなインタフェースを通じて機能を操作するために動きが用いられている。 \
        必要不可欠: 動作が機能に不可欠であり、この達成基準に従うと動作を無効化してしまう。'
    , "2_5_4.Devicemotion": 'この要素には devicemotion イベントリスナーがあります。デバイスの動きやユーザーの動きで操作できる機能は、ユーザーインターフェースコンポーネントでも操作可能であり、誤って操作してしまうことを防ぐために、その動きへの反応を無効にできることを確認してください。ただし、以下の場合は例外とします。 \
    サポートされたインタフェース: アクセシビリティ サポーテッドなインタフェースを通じて機能を操作するために動きが用いられている。 \
    必要不可欠: 動作が機能に不可欠であり、この達成基準に従うと動作を無効化してしまう。'


    //2_5_5.js
    , "2_5_5.Check": 'ポインタ入力の対象となるサイズは 44 x 44 CSS ピクセル以上であることを確認してください。ただし、以下の場合を除きます。 \
        同等のものが存在する: その対象と同等のリンクまたはコントロールが同じページに 44 x 44 CSS ピクセル以上のサイズで存在する。 \
        インラインである: その対象が文中、またはテキストブロック内に存在する。 \
        ユーザエージェントのコントロールである: 対象のサイズがユーザエージェントによって定められており、かつコンテンツ制作者が変更していない。 \
        必要不可欠: その対象を特定の方法で提示することが、情報伝達にとって必要不可欠である。'


    //2_5_6.js
    , "2_5_6.Check": '制限が不可欠な場合、コンテンツのセキュリティを確保するために必要な場合、またはユーザーの設定を尊重するために必要な場合を除き、コンテンツがプラットフォームで利用可能な入力モダリティの使用を制限していないことを確認します。'
    ...
    //4_1_3.js
    , "4_1_3_ARIA22,G199,ARIA19,G83,G84,G85,G139,G177,G194,ARIA23.Check": 'ステータスメッセージが、役割 (role) またはプロパティを通してプログラムによる解釈が可能であり、フォーカスを受けとらなくても支援技術によって利用者に提示されていることを確認してください。'
